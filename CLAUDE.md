...
FoodPilot is an automated pet feeding system running on a Raspberry Pi 4. An Angular SPA is served as static files by a FastAPI backend, which also exposes the REST API and controls hardware via GPIO.
...
```
backend/
├── main.py          # FastAPI app entry point; mounts API routers and serves static/
├── database.py      # SQLAlchemy engine + session factory + init_db()
├── api/             # Route handlers (feeding.py, status.py, history.py)
├── models/          # SQLAlchemy ORM models (feeding.py, status.py)
├── hardware/        # Hardware abstraction layer (dispenser.py, sensors.py)
└── static/          # Angular production build (served at /)

frontend/
└── src/app/
    ├── services/feeding.ts        # HttpClient wrapper for all /api/feeding/* calls
    ├── components/
    ...
    │   ├── status-placeholder/    # Route: /status (placeholder)
    │   └── history-placeholder/   # Route: /verlauf (placeholder)
...
**Hardware stub pattern**: `backend/hardware/dispenser.py` and `sensors.py` are logging-only stubs. All hardware interaction goes through these modules — replace stub bodies with GPIO calls once hardware is finalized. `SIZE_RUNTIME_SECONDS` in `dispenser.py` maps `small/medium/large` to motor run-time in seconds.
...
**Angular signals**: Components use Angular 22 signals (`signal()`, `computed()`) for local state rather than RxJS BehaviorSubjects.
